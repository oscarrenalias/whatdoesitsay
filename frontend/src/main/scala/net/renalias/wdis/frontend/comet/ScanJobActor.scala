...
import net.renalias.wdis.frontend.comet.{JobCompleted, RemoveJobListener, AddJobListener}
...
class ScanJobActor extends CometActor with Logger {
	...
	override def localSetup = {
		...
		registry.cometActorManager ! AddJobListener(jobId, this)
	}

	override def localShutdown = {
		info("Shutting down comet actor: " + {jobId})
		registry.cometActorManager ! RemoveJobListener(jobId, this)
	}
