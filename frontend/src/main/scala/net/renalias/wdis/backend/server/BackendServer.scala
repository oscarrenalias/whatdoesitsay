...
object BackendTestClient extends Logger {
	def main(args: Array[String]) = {
		val response = registry.backendServer !! Echo("Hello, world")
		info("The response from the server was = " + response.getOrElse("no response"))
	}
}
...
class BackendActor extends Actor with Logger {
	...
	info("BackendActor initializing")
	...
	def processJob(jobId: String) = {
		...
		val result = ScanJob.fetch(jobId) match {
			case Full(job) => info("Found!");processScanJob(job)
			case _ => {
				info("Not found!");
	...
	def receive = {
		case NewAsyncScanRequest(jobId) => registry.frontendServer ! processJob(jobId)
